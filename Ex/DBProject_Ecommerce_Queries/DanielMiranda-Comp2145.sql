...
DECLARE
  ...
  v_status_date DATE;
BEGIN
  -- Test with basket ID 1005
  STATUS_SP(1005, v_status_desc, v_status_date);
  DBMS_OUTPUT.PUT_LINE('Basket ID: 1005');
  DBMS_OUTPUT.PUT_LINE('Status Description: ' || v_status_desc);
  DBMS_OUTPUT.PUT_LINE('Status Date: ' || v_status_date);
END;

--This Procedure looking for the shiiping cost
...
VALUES (1010, 1001, 100, 'Shelf 707'); -- Inserting a product with ID  and initial quantity of 100
...
SELECT * FROM orderitem WHERE ProductID = 1000;
...
SELECT *
FROM all_triggers
WHERE trigger_name = 'UPDATE_ORDER_STATUS';

  
  BEGIN
   COMP214_M23_zo_17.UPDATE_ORDER_STATUS(1000, 04-12-23);
END;
